...
from utils.common_util import AverageMeter, intersectionAndUnion, check_makedirs
...
def test(model, criterion, names):
    ...
    for idx, item in enumerate(data_list):
        ...
        if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
            # if already exist,  get pred and label directly from file
            ...
            # else get predict result by model inference
            coord, feat, label, idx_data = data_load(item)  # idx_data: [0, ..., NumPoints - 1]
            # initial predict results
            ...
            for i in range(batch_num):
                ...
                logger.info('Test: {}/{}, {}/{}, {}/{}'.format(idx + 1, len(data_list), e_i, len(idx_list),
                                                               args.voxel_max, idx_part.shape[0]))

            loss = criterion(pred, torch.LongTensor(label).cuda(non_blocking=True))  # for reference
            pred = pred.max(1)[1].data.cpu().numpy()  # get predict class index
    ...
    # Calculation 2  TODO: diff with calculate 1?
...
if __name__ == '__main__':
    ...
    names = [line.rstrip('\n') for line in open(args.names_path)]  # part names
