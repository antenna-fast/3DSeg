...
class S3DIS(Dataset):
    def __init__(self, split='train', data_root='trainval',
                 test_area=5, voxel_size=0.04, voxel_max=None,
        ...
        for item in self.data_list:
            if not os.path.exists("/dev/shm/{}".format(item)):
                ...
                data = np.load(data_path)  # xyzrgb, label, N*7
    ...
    def __getitem__(self, idx):
        ...
        coord, feat, label = data[:, 0:3], data[:, 3:6], data[:, 6]  # coord, rgb, label
        coord, feat, label = data_prepare(coord, feat, label, self.split, self.voxel_size,
                                          self.voxel_max, self.transform, self.shuffle_index)
