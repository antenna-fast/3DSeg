...
class S3DIS(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096,
        ...
        for room_name in rooms_split:
            ...
            points, labels = room_data[:, 0:6], room_data[:, 6]  # xyzrgb, N*6; label, N
            coord_min, coord_max = np.amin(points, axis=0)[:3], np.amax(points, axis=0)[:3]
            self.room_points.append(points), self.room_labels.append(labels)
            self.room_coord_min.append(coord_min), self.room_coord_max.append(coord_max)
            num_point_all.append(labels.size)
        sample_prob = num_point_all / np.sum(num_point_all)  # points value / points num
    ...
    def __getitem__(self, idx):
        # self.logger.info('item: {}'.format(idx))
        ...
        while True:
            center = points[np.random.choice(N_points)][:3]  # TODO: random sample as center?
            block_min = center - [self.block_size / 2.0, self.block_size / 2.0, 0]
            block_max = center + [self.block_size / 2.0, self.block_size / 2.0, 0]
            point_idxs = np.where((points[:, 0] >= block_min[0]) &  # 在范围内的点的索引
        ...
        current_points[:, 0:6] = selected_points  # [centralized xyz, normalized rgb]
