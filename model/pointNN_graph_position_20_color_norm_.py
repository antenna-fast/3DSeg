...
class NN(nn.Module):
    ...
    def forward(self, x):
        ...
        colors = x[:, :, 3:6] / 255  # color normalize
        ...
        for sample_idx, (p, c) in enumerate(zip(points, colors)):
            ...
            # nbrs: [N, k]
            nbrs_flat = nbrs.reshape(-1)  # [N, k] to Nxk,
            points_knn_flat = p[nbrs_flat].reshape(num_points, -1, 3)  # reshape: to [N, k, 3]
            ...
            p_center[:, 0, :] = torch.tensor([0, 0, 0], dtype=torch.float)  # idx_0 to zero, preserve itself
            knn_edge = points_knn_flat - p_center  # [N, k, 3], [position, knn_edges]
            batch_edge_feature.append(knn_edge.numpy())  # patch to batch
