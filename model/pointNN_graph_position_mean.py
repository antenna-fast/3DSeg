...
def get_graph_feature(x, k=20, idx=None):
    # build graph using feature's knn
...
class NN(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim):
        ...
        self.linear_bn1 = nn.BatchNorm1d(128)
    ...
    def forward(self, x):
        ...
        for sample_idx, (p, c) in enumerate(zip(points, colors)):
            ...
            colors_knn_flat = c[nbrs_flat].reshape(num_points, -1, 3)
            batch_knn_colors.append(colors_knn_flat)
        ...
        x1 = x.mean(dim=-1, keepdim=False)  # [batch, feat_dim, N]  # average pooling
        x1 = x1.permute(0, 2, 1)  # [batch, N, feat_dim]

        x = self.shared_mlp(x1)
        # x = self.shared_mlp(x2)
        return x
