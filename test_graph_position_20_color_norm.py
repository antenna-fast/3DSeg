    ...
    for idx, data_name in enumerate(data_list):  # file name
        ...
        if args.is_vis:
            ...
            show_inference(coord, color_pred_np, is_show=0, is_save=1, is_norm=1,
                           save_path=os.path.join(pcd_save_root, 'pred_scene_{}.ply'.format(log_idx)))
            show_inference(coord, color_gt_np, is_show=0, is_save=1, is_norm=1,
                           save_path=os.path.join(pcd_save_root, 'gt_scene_{}.ply'.format(log_idx)))
            show_inference(coord, feat, is_show=0, is_save=1, is_norm=1,
                           save_path=os.path.join(pcd_save_root, 'color_scene_{}.ply'.format(log_idx)))
...
if __name__ == '__main__':
    ...
    config_file_path = os.path.join(code_root, 'config/s3dis/s3dis_antenna_graph_position_20_color_norm.yaml')  # YAML config file
